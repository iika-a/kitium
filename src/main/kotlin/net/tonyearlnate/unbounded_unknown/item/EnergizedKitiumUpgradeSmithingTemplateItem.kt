...
class EnergizedKitiumUpgradeSmithingTemplateItem(settings: Settings) : SmithingTemplateItem(
    ...
    companion object {
        ...
        private val UPGRADE_APPLIES_TO_TEXT: Text = Text.translatable(
            Util.createTranslationKey("item", Identifier.of(UnboundedUnknown.MOD_ID, "smithing_template.energized_kitium_upgrade.applies_to"))
        ).formatted(DESCRIPTION_FORMATTING)

        private val UPGRADE_INGREDIENTS_TEXT: Text = Text.translatable(
            Util.createTranslationKey("item", Identifier.of(UnboundedUnknown.MOD_ID, "smithing_template.energized_kitium_upgrade.ingredients"))
        ).formatted(DESCRIPTION_FORMATTING)

        private val UPGRADE_BASE_SLOT_DESCRIPTION_TEXT: Text = Text.translatable(
            Util.createTranslationKey("item", Identifier.of(UnboundedUnknown.MOD_ID, "smithing_template.energized_kitium_upgrade.base_slot_description"))
        )

        private val UPGRADE_ADDITIONS_SLOT_DESCRIPTION_TEXT: Text = Text.translatable(
            Util.createTranslationKey("item", Identifier.of(UnboundedUnknown.MOD_ID, "smithing_template.energized_kitium_upgrade.additions_slot_description"))
        )
